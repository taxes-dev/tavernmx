...
namespace tavernmx::client
{
	...
	void ClientUi::render_connect() {
		...
		ImGui::SameLine();
		if (ImGui::InputText("##user", &state_bag["user"s], ImGuiInputTextFlags_EnterReturnsTrue)) {
			this->call_handler(ClientUiMessage::Connect_ConnectButton);
		}
		ImGui::Text("Host:");
		ImGui::SameLine();
		if (ImGui::InputText("##host", &state_bag["host"s], ImGuiInputTextFlags_EnterReturnsTrue)) {
			this->call_handler(ClientUiMessage::Connect_ConnectButton);
		}
		ImGui::Text("Port:");
		ImGui::SameLine();
		if (ImGui::InputText("##port", &state_bag["port"s],
	...
	void ClientUi::render_chat_window() {
		...
		this->window_center();
		ImGui::Begin(state_bag["host"s].c_str(), nullptr);
		ImGui::End();
	}
