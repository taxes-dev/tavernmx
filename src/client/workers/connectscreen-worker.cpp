...
namespace tavernmx::client
{
	...
	void connect_screen_worker(ConnectUiScreen* connect_screen, const ClientConfiguration& config) {
		...
		connect_screen->add_handler(
			ConnectUiScreen::MSG_CONNECT_BUTTON, [&config](ClientUi* ui, ClientUiScreen* screen) {
				...
				try {
					...
					connection =
						std::make_unique<ServerConnection>(conn_screen->host_name, host_port, conn_screen->user_name);
					...
					std::thread connection_thread{ []() {
						try {
							...
							if (const std::optional<Message> acknak = connection->wait_for_ack_or_nak();
								acknak && acknak->message_type == MessageType::NAK) {
								auto nakmsg = message_value_or<std::string>(*acknak, "error");
								TMX_WARN("Server denied request to connect: {}", nakmsg);
								connect_thread_error = std::move(nakmsg);
								connection->shutdown();
							} else if (acknak && acknak->message_type == MessageType::ACK) {
								TMX_INFO("Server acknowledged HELLO");
							} else {
					...
					connecting_screen->add_handler(ConnectingUiScreen::MSG_UPDATE, connecting_screen_update);
					ui->push_screen(std::move(connecting_screen));
				} catch (std::exception& ex) {
					connection.reset();
					ui->set_error(ex.what());
				}
			});
	}
}
